        ...
        let features_vec: Result<Vec<Feature>> = match self {
            Query::VecExpr(query) => {
                let features_result: Result<Vec<_>> = query
                    .iter()
                    .map(|expr_str| {
                        Feature::from_str(expr_str)
                            .map_err(|_| anyhow!("Failed to parse feature from string"))
                    })
                    .collect();
...
impl Default for EventContext {
    fn default() -> Self {
        EventContext {
            ...
            settings: EventStoreSettings::default(),
        }
    }
}

impl EventContext {
    pub fn new_memory(settings: EventStoreSettings) -> EventContext {
        Self {
            event_store: EventStoreImpl::MemoryEventStore(MemoryEventStore::new(settings.clone())),
    ...
    pub fn extract_records_from_expr(
        ...
        let results: Vec<_> = match (self.settings.parallel, chunk_size) {
