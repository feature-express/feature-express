...
#[derive(Clone, Debug)]
pub struct ArgMax<T> where T: Ord + Clone {
    count: usize,
    state: BTreeMap<OrderedFloat<FLOAT>, Vec<T>>, // value -> timestamps
}

impl<T> PartialAggregate for ArgMax<T> where T: Ord + Clone {
...
impl<T> SubtractPartialAggregate for ArgMax<T> where T: Ord, T: Clone {
