...
use crate::event_index::{check_agg_event_type_index, EventContext, EventQueryConfig};
...
pub struct EvalContext<'a> {
    ...
    pub event_query_config: EventQueryConfig,
...
pub fn eval_simple_expr(
    ...
    let result = match expr {
        ...
        | Expr::AttrDateTime(attribute) => Ok(event_with_context?
            .extract_attribute(attribute)
            .context("Cannot extract attribute")?),
...
pub fn extract_interval_events(
    ...
    match &context.event_query_config {
        EventQueryConfig::RelatedEntitiesEvents(selected_entities) => {
        ...
        EventQueryConfig::AllEvents => context.event_index.event_store.query_interval(interval),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_agg() {
        let mut event_context = EventContext::new_memory(EventStoreSettings::default());
        ...
        let event_query_config =
            EventQueryConfig::RelatedEntitiesEvents(vec![EntityType("1".into())]);
