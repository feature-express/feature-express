...
mod tests {
    ...
    use crate::event_store::{EventStore, EventStoreSettings};
    ...
    fn test_consistent_counts() {
        ...
        let entity_query = EventQueryConfig::RelatedEntitiesEvents(vec![EntityType("user".into())]);
    ...
    fn generate_event_context(
        n_events: usize,
        event_store_settings: EventStoreSettings,
    ) -> EventContext {
        let events = generate_random_events(n_events);
        let mut event_context = EventContext::new_memory(event_store_settings);
