...
pub const EPS: FLOAT = 1.0e-12;

fn local_cmp(x: FLOAT, y: FLOAT) -> Ordering {
...
fn local_sort(v: &mut [FLOAT]) {
    v.sort_by(|x: &FLOAT, y: &FLOAT| local_cmp(*x, *y));
}
...
pub trait Stats {
    ...
    fn median_abs_dev(&self) -> FLOAT;

    /// Median absolute deviation as a percent of the median. See `median_abs_dev` and `median`.
    fn median_abs_dev_pct(&self) -> FLOAT;
    ...
    fn percentile(&self, pct: FLOAT) -> FLOAT;
    ...
    fn quartiles(&self) -> (FLOAT, FLOAT, FLOAT);
...
pub struct Summary {
    ...
    pub var: FLOAT,
    pub std_dev: FLOAT,
    pub std_dev_pct: FLOAT,
    pub median_abs_dev: FLOAT,
    pub median_abs_dev_pct: FLOAT,
    pub quartiles: (FLOAT, FLOAT, FLOAT),
    pub iqr: FLOAT,
}

impl Summary {
    /// Construct a new summary of a sample set.
    pub fn new(samples: &[FLOAT]) -> Summary {
        Summary {
            sum: samples.sum(),
            min: samples.min(),
            max: samples.max(),
...
impl Stats for [FLOAT] {
    ...
    fn std_dev_pct(&self) -> FLOAT {
        let hundred = 100 as FLOAT;
        (self.std_dev() / self.mean()) * hundred
    }

    fn median_abs_dev(&self) -> FLOAT {
        let med = self.median();
        let abs_devs: Vec<FLOAT> = self.iter().map(|&v| (med - v).abs()).collect();
    ...
    fn quartiles(&self) -> (FLOAT, FLOAT, FLOAT) {
        let mut tmp = self.to_vec();
        local_sort(&mut tmp);
        let first = 25 as FLOAT;
        let a = percentile_of_sorted(&tmp, first);
        let second = 50 as FLOAT;
        let b = percentile_of_sorted(&tmp, second);
        let third = 75 as FLOAT;
...
fn percentile_of_sorted(sorted_samples: &[FLOAT], pct: FLOAT) -> FLOAT {
    ...
    let hundred = 100 as FLOAT;
    assert!(pct <= hundred);
    if (pct - hundred).abs() < EPS {
        return sorted_samples[sorted_samples.len() - 1];
    }
    let length = (sorted_samples.len() - 1) as FLOAT;
...
pub fn winsorize(samples: &mut [FLOAT], pct: FLOAT) {
...
// Test vectors generated from R, using the script fexpress_python-core/etc/stat-test-vectors.r.
...
mod tests {
    ...
    //noinspection ALL
    fn check(samples: &[FLOAT], summ: &Summary) {
    ...
    #[test]
    fn test_min_max_nan() {
        let xs = &[1.0, 2.0, FLOAT::NAN, 3.0, 4.0];
    ...
    fn test_sum_floats() {
        assert_eq!(
            [0.5 as FLOAT, 3.2321 as FLOAT, 1.5678 as FLOAT].sum(),
            5.2999
        );
    }

    #[test]
    fn test_sum_float_between_ints_that_sum_to_0() {
        assert_eq!([1e30 as FLOAT, 1.2 as FLOAT, -1e30 as FLOAT].sum(), 1.2);
    }
}
