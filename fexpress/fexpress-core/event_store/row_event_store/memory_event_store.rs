...
use crate::event_index::{EventContext, EventQueryConfig};
use crate::event_store::{EventStore, EventStoreImpl, EventStoreSettings};
...
pub struct MemoryEventStore {
    ...
    pub settings: EventStoreSettings,
}
...
impl MemoryEventStore {
    pub fn new(settings: EventStoreSettings) -> Self {
        Self {
            sm: Default::default(),
    ...
    fn extract_events_from_treemap(
        ...
        let range = interval.map(|interval| {
            let (start_dt, end_dt) = if self.settings.include_events_on_obs_date {
...
impl EventStore for MemoryEventStore {
        ...
        let events_without_experiment = entities
            ...
            .map(|(entity_type, entity_id)| {
                index_by_entity_event_type_ts
                    ...
                    .map(|treemap| {
                        self.extract_events_from_treemap(Some(interval), treemap)
                            .ok()
                    })
                    .flatten()
            })
    ...
    fn query_entity_interval(
        ...
        let global_events = self
            .extract_events_from_treemap(Some(interval), &global_index_ts)
            .ok();
    ...
    fn query_entity(
        ...
        let events_without_experiment = entities
            ...
            .map(|(entity_type, entity_id)| {
                index_by_entity_ts
                    ...
                    .map(|treemap| self.extract_events_from_treemap(None, treemap).ok())
                    .flatten()
            })
            .collect_vec();
        let events_without_experiment = intersect(events_without_experiment);
        ...
        let events_global = self
            .extract_events_from_treemap(None, &global_index_by_ts)
            .ok();
...
mod test {
    ...
    fn test_event_filter() {
        ...
        let events = event_db.filter_events(&expr, &hm).unwrap();
    }
    ...
    pub fn test_query_event_type_entity() {
        ...
        let entity_query =
            EventQueryConfig::RelatedEntitiesEvents(vec![EntityType("".to_string())]);
        let features = store
    ...
    fn test_query_event_type() {
        ...
        let result = store
            .event_store
            .query_event_type(&event_type, Some(&interval))
